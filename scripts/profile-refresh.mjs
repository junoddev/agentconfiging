...
const result = spawnSync('npm', args, { encoding: 'utf8', env: { ...process.env, NO_COLOR: '1' } });
...
fs.writeFileSync(
  ...
  `${JSON.stringify({ schemaVersion: 1, profileId: profile, mode, exitCode: code, complete: (code === 0 || code === 1) && (report?.unresolvedSourceIds?.length ?? 0) === 0, unresolvedSourceIds: report?.unresolvedSourceIds ?? [] })}\n`,
);
